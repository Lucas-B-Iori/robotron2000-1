...
const estatiscas = document.querySelectorAll('[data-estatisca]');

controle.forEach((elemento) => {
  elemento.addEventListener("click", (evento) => {
    manipulaDados(evento.target.dataset.controle, evento.target.parentNode);
    atualizaEstatica(evento.target.dataset.peca);
  });
});
...
function atualizaEstatica(peca) {
    estatiscas.forEach((elemento) => {
        elemento.textContent = parseInt(elemento.textContent) + pecas[peca][elemento.dataset.estatisca];
    })
}
